...
<div class="title-btn left"><a href="/fruits_shop/Admin/Goods/add/cid/<?php echo ($cid); ?>">添加商品</a></div>
...
<table border="1">
        <form action="/fruits_shop/Admin/Goods/delSelect/cid/<?php echo ($cid); ?>" method="post">
            <tr><th width="120">商品编号</th><th>商品名</th><th width="80"><a 
                        href="/fruits_shop/Admin/Goods/index/cid/<?php echo ($cid); ?>/status/<?php echo empty($get_order['status'])?'desc':$get_order['status']; ?>">上架</a></th><th width="80"><a href="/fruits_shop/Admin/Goods/index/cid/<?php echo ($cid); ?>/is_best/<?php echo empty($get_order['is_best'])?'desc':$get_order['is_best']; ?>">推荐</a></th><th width="120">操作</th></tr>
            <?php if(is_array($goods["list"])): foreach($goods["list"] as $key=>$v): ?><tr>
                    <td><input type="checkbox" name="gid[]" value="<?php echo ($v["gid"]); ?>"/><?php echo ($v["identifier"]); ?></td>
                    <td><?php echo ($v["gname"]); ?></td>
                    <td class="center"><a href="/fruits_shop/Admin/Goods/change/status/<?php echo ($v["status"]); ?>/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>"><?php if(($v["status"]) == "yes"): ?>是<?php else: ?>否<?php endif; ?></a></td>
                    <td class="center"><a href="/fruits_shop/Admin/Goods/change/is_best/<?php echo ($v["is_best"]); ?>/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>"><?php if(($v["is_best"]) == "yes"): ?>是<?php else: ?>否<?php endif; ?></a></td>
                    <td class="center"><a href="/fruits_shop/Admin/Goods/revise/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>">修改</a> <a href="/fruits_shop/Admin/Goods/del/gid/<?php echo ($v["gid"]); ?>/cid/<?php echo ($cid); ?>">删除</a></td>
...
<script>
    $("select").change(function () {
        window.location.href = "/fruits_shop/Admin/Goods/index/cid/" + $(this).val();
    });
    $(function () {
        $("tr:odd").addClass("odd");
    });
